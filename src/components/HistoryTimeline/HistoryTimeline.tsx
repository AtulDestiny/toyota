...
export const ToyotaHistoryTimeline: React.FC<ToyotaHistoryTimelineProps> = (
  props
) => {
  ...
  const yearStyle = props.yearStyle || {};
  ...
  const renderPeriodHeader = () => {
    const startYear = hasPeriodData ? periodStartYear : derivedPeriodStartYear;
    const endYear = hasPeriodData ? periodEndYear : derivedPeriodEndYear;
    ...
    return (
      <View marginBottom="2rem">
        ...
        <View
          ...
          padding={{ base: ".9375rem", medium: "1.5rem" }}
          borderRadius="20px"
          marginTop="1rem"
          maxWidth="500px"
          width={{ base: "294px", medium: "auto" }}
          minHeight={{ base: "111px", medium: "auto" }}
          border="none"
          margin="1rem auto 0 auto"
        >
          <Flex justifyContent="center" alignItems="center" direction="column">
            {/* Icon (trophy, steering wheel, etc.) */}
            {(periodIcon || props.items[0]?.icon) && (
              <Image
                src={periodIcon || props.items[0]?.icon}
                alt={periodIconAlt}
                width={{ base: "25px", medium: "36px" }}
                height={{ base: "25px", medium: "36px" }}
                marginBottom={{ base: "0", medium: "1rem" }}
              />
            )}
            ...
            <Text
              ...
              textAlign="center"
              fontWeight="400"
              marginBottom={{ base: "0", medium: "1.5rem" }}
            >
              {periodSummary || props.items[0]?.description || ""}
            </Text>

            {/* Toggle Button - smaller and less prominent */}
          </Flex>
        </View>
        <div
          style={{
  ...
  return (
    ...
    >
      <View
        maxWidth="1200px"
        margin="0 auto"
        padding={resolveResponsiveValue(containerStyle.padding) || "0"}
      >
        {/* Period Header */}
        {renderPeriodHeader()}
